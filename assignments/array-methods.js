...
let priceArray = [];
runners.forEach(runner => priceArray.push(runner.donation));

let ticketPriceTotal = priceArray.reduce(function(acc, curVal){
  return acc + curVal;
}, 0);
...
// Problem 1
//The shirts are smaller than expected! Bump everybody's size up one.
let biggerShirts = [];
biggerShirts = runners.map( runner => {
    new_size = "";
    if (runner.shirt_size === 'XS') {
        new_size = 'S';
        return {...runner, new_size}
...
biggerShirts.forEach(runner => {
  console.log(runner.last_name + ' shirt size now: ' + runner.new_size);
})

// Problem 2
//Oh no! It's gonna rain! Give me everybody's email so we can move the date
const runnersEmails = [];
runners.map((runner) => runnersEmails.push(runner.email));
console.log(runnersEmails);


// Problem 3
//Let's create our runner tags! Put the id number, last name, and company name into one string each
const runnerTags = [];
runners.map(runner => runnerTags.push(runner.id + ' ' + runner.last_name + ' ' + runner.company_name));
console.log(runnerTags);